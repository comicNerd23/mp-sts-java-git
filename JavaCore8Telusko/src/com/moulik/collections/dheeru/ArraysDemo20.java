...
/**
 * Arrays class:
 * Arrays is a class which supports many utility operations that work well with java.util.Collection objects.
 * 
 * When we want to convert an array to a list, we use Arrays.asList(array).
 * This list is a fixed-size list. That means, adding/removing elements will result in UnsupportedOperationException.
 * But we can update/set the value.
 * 
 * When we want to convert an array to a modifiable list, we can pass that list as an argument to ArrayList constructor.
