package com.moulik.threads.dheeru.waitnotifywithincompleteinterruption11;
...
private Deque<Weblink> queue = new ArrayDeque<>();
	...
	private static class Weblink {
	    ...
	    //To check the status of thread if it has been stopped/interrupted or not
			...
			try {
				synchronized (weblink) {
					...
					try {
						...
						bgThread.interrupt(); //Even on successful download, We are calling interrupt() to terminate this thread because we don't want to keep 
											  //it running.
